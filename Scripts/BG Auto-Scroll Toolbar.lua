...
--©2019 - 2021 CarbsCode Please don't Claim this script as your own work <3
...
if app.apiVersion ==  nil or app.apiVersion  < 12 then
	...
	local required_version = "1.2.22 (Script API 12) or newer"
	app.alert{
...
infoDLG = Dialog{title="Help & Info",onclose=function() shSBGHelp(true)end}
	...
	:separator{text=""}
	:newrow()
	:label{text="BG Auto-Scroll Version 1.1.4 | CarbsCode.itch.io | © CarbsCode 2019 - 2021 All Rights Reserved"}


settingsDLG = Dialog{title="BG Auto-Scroll v1.1.4",onclose=function() toolbarSBGClosed()end}
	...
	if dir1 ~= nil then
		diag = true -- if we do have a second direction then this is a diagonal.
	  	...
	  	if aFrame == nil then
	  		...
	  		local sprH = frameSpr.height
	  		local PPFx = sprW / (frameTime/loopMultiplier)--Pixels Per Frame in the x
	  		local PPFy = sprH/ (frameTime/loopMultiplier)--Pixels Per Frame in the y
	  	--Create new frames
